...
codetypo_path=`$current_dir/uut/codetypo.sh path $base_dir`
codetypo_version=`$current_dir/uut/codetypo.sh version $base_dir`
print_tool "codetypo" "$codetypo_version" "$codetypo_path" "$report_path"
...
function bench_dir() {
  ...
  if [[ -z $path ]]; then
    ...
    codetypo_command=""
    codetypo_ascii_command=""
    codetypo_j1_command=""
    if [[ ! -z $codetypo_path ]]; then
      codetypo_command="$codetypo_path $path"
      codetypo_ascii_command="$codetypo_path --no-unicode $path"
      codetypo_j1_command="$codetypo_path --threads 1 $path"
    fi
    misspell_go_command=""
    ...
    hyperfine --warmup 1 -i --export-json $report_prefix-rg.json --export-markdown $report_prefix-rg.md "$rg_command" "$rg_j1_command" "$codetypo_command" "$codetypo_ascii_command" "$codetypo_j1_command" "$misspell_go_command" "$codespell_command"
...
function bench_file() {
  ...
  if [[ -z $path ]]; then
    ...
    hyperfine --warmup 1 -i --export-json $report_prefix-rg.json --export-markdown $report_prefix-rg.md "$rg_command" "$codetypo_command" "$codetypo_ascii_command" "$misspell_go_command" "$scspell_command" "$codespell_command"
    cat $report_prefix-rg.md >> $output
  fi
  echo "" >> $output
}
