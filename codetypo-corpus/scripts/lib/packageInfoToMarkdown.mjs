...
function listDictionariesByCategory(packages) {
    ...
    md +=
        '\n\n' +
        '<sup>1</sup> Bundled with CodeTypo.<br>' +
...
`;
    ...
    for (const dict of dictionaries) {
        const codetypo = dict.pkg.codetypo ? ' <sup>1</sup>' : '';
        ...
        md += `| [\`${dict.name}\`](${dict.pkg.dir})${codetypo}${enabled} | ${dict.description} | ${locales} | ${fileTypes} |\n`;
    }
...
function formatPackageRow(pkg) {
    ...
    // | Package | Name | Dictionary IDs |
    return `| [${packageName}](./${dir}#readme)${pkg.codetypo ? '<sup>1</sup>' : ''} | ${pkg.name} | ${dictNames} |`;
}
...
function formatPackage(pkg) {
    return `- [${pkg.name}](${pkg.dir}) - ${pkg.description} ${pkg.codetypo ? '<sup>1</sup>' : ''} ${pkg.hasEnabledByDefault ? '<sup>2</sup>' : ''}`;
}
