...
const codetypoArgsListAllFiles =
    '--no-progress --relative --show-context --gitignore --gitignore-root=. --reporter=default --reporter=${pathReporterListAll}';
const jsCodetypo = JSON.stringify(Path.resolve(__dirname, '../../bin.mjs'));

const envVariables: string[] = [
    'CODETYPO_ENABLE_DICTIONARY_LOGGING',
    'CODETYPO_ENABLE_DICTIONARY_LOG_FILE',
    'CODETYPO_ENABLE_DICTIONARY_LOG_FIELDS',
    'CODETYPO_GLOB_ROOT',
];
...
async function execCheck(context: CheckContext, update: boolean): Promise<CheckResult> {
    ...
    const cmdArgs = rep.listAllFiles ? codetypoArgsListAllFiles : codetypoArgs;
    const cmdToExec = resolveArgs(rep.path, [genLaunchCodeTypoCommand(nodeArgs), cmdArgs, ...uniqueArgs]).join(' ');
    const { log } = logger;
    ...
    log(time());
    const codetypoResult = await execCommand({ logger, path, command: cmdToExec, args: rep.args, env });
    log(resultReport(codetypoResult));
    log(time());
    log(color`\n************ Checking Results ************`);
    ...
    const r = await checkResult(rep, codetypoResult, originalReport, update);
    log(time());
    if (r.diff) {
        log(r.diff);
        log('');
    ...
    log(color`\n************ Done: ${name} ************\n`);
    return { success: r.match, rep, elapsedTime: codetypoResult.elapsedTime };
}
...
function genLaunchCodeTypoCommand(nodeArgs: string[]) {
    return `node ${nodeArgs.join(' ')} ${jsCodetypo}`;
}
