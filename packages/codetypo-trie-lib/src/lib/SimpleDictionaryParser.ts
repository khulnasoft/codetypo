import type { Operator } from '@codetypo/codetypo-pipe/sync';
import { opCombine as opPipe, opConcatMap, opFilter, opMap } from '@codetypo/codetypo-pipe/sync';
...
export const codeTypoToolDirective = 'codetypo-dictionary:';

export const setOfCodeTypoDirectiveFlags = ['no-split', 'split', 'generate-alternatives', 'no-generate-alternatives'];
...
export function createDictionaryLineParserMapper(options?: Partial<ParseDictionaryOptions>): Operator<string> {
    ...
    function removeComments(line: string): string {
        ...
        const idxDirective = line.indexOf(codeTypoToolDirective, idx);
        if (idxDirective >= 0) {
            const flags = line
                .slice(idxDirective)
                .split(/[\s,;]/g)
