...
import type { CodeTypoSettingsWithSourceTrace, CodeTypoUserSettings } from '@codetypo/codetypo-types';
import { ICodeTypoConfigFile } from 'codetypo-config-lib';
...
describe('Validate Spell Checking Documents', async () => {
    ...
    const sampleConfigFile = await readConfigFile(rpS('../codetypo.config.json'));
...
function sanitizeSpellCheckFileResult(
    ...
    cfgKeys?: (keyof CodeTypoUserSettings)[],
...
function sanitizeSettings(
    cs: CodeTypoSettingsInternal | CodeTypoSettingsWithSourceTrace,
    keys: (keyof CodeTypoUserSettings)[] = ['languageId'],
...
function toConfigFile(settings: CodeTypoUserSettings, url = 'codetypo.json') {
    return {
...
async function readConfigFile(filename: string): Promise<ICodeTypoConfigFile> {
