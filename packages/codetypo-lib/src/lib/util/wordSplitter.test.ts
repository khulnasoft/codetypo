import type { TextOffset } from '@codetypo/codetypo-types';
...
describe('Validate wordSplitter', () => {
    ...
    test('expensive split', () => {
        ...
        // codetypo:ignore VQOZ Mfsptgkk ojid HAEN
    ...
    // codetypo:ignore CVTPD CVTSI CVTTSD words'separated'by errorcode
    // codetypo:word Geschäft gescha
    test.each`
        text                                | expectedWords
    ...
    // codetypo:ignore nstatic techo n'cpp n'log refactor'd î
    test.each`
        text                  | expectedWords      | calls
        ${'static'}           | ${'static'}        | ${1}
        ${'nstatic'}          | ${'static'}        | ${1}
...
// codetypo:ignore êphone îphone geschäft
