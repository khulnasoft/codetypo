import type { CodeTypoSettings, DictionaryId, LocaleId } from '@codetypo/codetypo-types';
...
export async function traceWords(
    words: string[],
    settings: CodeTypoSettings | ICodeTypoConfigFile,
...
export async function* traceWordsAsync(
    ...
    settingsOrConfig: CodeTypoSettings | ICodeTypoConfigFile,
    options: TraceOptions | undefined,
): AsyncIterableIterator<TraceWordResult> {
    const { languageId, locale: language, ignoreCase = true, allowCompoundWords } = options || {};

    const settings = satisfiesCodeTypoConfigFile(settingsOrConfig)
        ? await resolveConfigFileImports(settingsOrConfig)
        : settingsOrConfig;

    async function finalize(config: CodeTypoSettings): Promise<{
