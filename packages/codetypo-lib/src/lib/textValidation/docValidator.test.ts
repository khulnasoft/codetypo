...
describe('docValidator', () => {
    ...
    // codetypo:ignore kount naame colector Reciever reciever recievers serrors dockblock
    test.each`
        ...
        ${tFix('issues/issue-4811/#local/version@2.md')} | ${undefined}         | ${['marrkdown']}                                                                                                                                                                              | ${undefined /* codetypo:disable-line */}
    `(
    ...
    test.each`
        ...
        ${fix('sample-with-common-errors.md')} | ${expect.arrayContaining([mkI('acceptible', 'acceptable'), mkI('accidently', 'accidentally')]) /* codetypo:disable-line */}
    ...
    test('updateDocumentText', async () => {
        ...
        const r = dVal.checkDocument();
        expect(r).toEqual(expectedIssues);
        await dVal.updateDocumentText(doc.text + '# codetypo:ignore foor\n');
...
function s(...settings: CodeTypoUserSettings[]): CodeTypoUserSettings {
