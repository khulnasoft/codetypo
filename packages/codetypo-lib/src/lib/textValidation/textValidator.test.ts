...
import type { CodeTypoUserSettings, TextOffset } from '@codetypo/codetypo-types';
...
function sToV(settings: CodeTypoUserSettings) {
    return settingsToValidateOptions(finalizeSettings(settings));
}

// codetypo:ignore whiteberry redmango lightbrown redberry
...
describe('Validate textValidator functions', () => {
    ...
    // codeTypo:ignore xxxkxxxx xxxbxxxx
    ...
    test('tests case in ignore words', async () => {
        const dict = await getDictionaryInternal(
            csi({
                ...
                ignoreWords: ['PUBLISHed', 'FIXesToThePROBLEMs'], // codetypo:ignore fixestotheproblems
            }),
        );
        const text =
