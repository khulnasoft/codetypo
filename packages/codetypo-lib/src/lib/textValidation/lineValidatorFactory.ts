...
import { opConcatMap, opFilter, pipe } from '@codetypo/codetypo-pipe/sync';
import type { ParsedText } from '@codetypo/codetypo-types';
import { defaultCodeTypoSettings } from '@codetypo/codetypo-types';
import type { CachingDictionary, SearchOptions, SpellingDictionary } from 'codetypo-dictionary';
import { createCachingDictionary } from 'codetypo-dictionary';
...
export function lineValidatorFactory(sDict: SpellingDictionary, options: ValidationOptions): LineValidator {
    const {
        ...
        ignoreCase = true,
        ignoreRandomStrings = defaultCodeTypoSettings.ignoreRandomStrings,
        minRandomLength = defaultCodeTypoSettings.minRandomLength,
    } = options;
    ...
    const regExUpperCaseWithTrailingCommonEnglishSuffix =
        /^([\p{Lu}\p{M}]{2,})['’]?(?:s|ing|ies|es|ings|ize|ed|ning)$/u; // codetypo:disable-line
    ...
    const fn: LineValidatorFn = (lineSegment: LineSegment) => {
         ...
         *
         * Note: See [#6066](https://github.com/khulnasoft/codetypo/pull/6066)
         * Using just `regExpCamelCaseWordBreaks` misses unknown 4-letter words.
         *
         * The code below was tried, but it missed words.
         * - `LSTM` was caught. // codetypo:disable-line
         * - `LSTMs` was missed because it becomes `LST` and `Ms`. // codetypo:disable-line
         *
         * ```ts
