...
import { opConcatMap, opMap, pipeSync } from '@codetypo/codetypo-pipe/sync';
import type {
...
import { IssueType } from '@codetypo/codetypo-types';
import { toFilePathOrHref, toFileURL } from '@codetypo/url';
...
export class DocumentValidator {
    ...
    static async create(
        ...
        settingsOrConfigFile: CodeTypoUserSettings | ICodeTypoConfigFile,
    ): Promise<DocumentValidator> {
        const settings = satisfiesCodeTypoConfigFile(settingsOrConfigFile)
    ...
    public getFinalizedDocSettings(): CodeTypoSettingsInternal {
        assert(this._ready);
        assert(this._preparations, ERROR_NOT_PREPARED);
...
async function searchForDocumentConfig(
    ...
    defaultConfig: CodeTypoSettingsWithSourceTrace,
    pnpSettings: PnPSettings,
): Promise<CodeTypoSettingsWithSourceTrace> {
