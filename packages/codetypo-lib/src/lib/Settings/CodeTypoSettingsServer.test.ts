...
describe('Validate CodeTypoSettingsServer', () => {
    ...
    test('tests mergeSettings with ignorePaths, files, and overrides', () => {
        ...
        const right = csi({
            ...
            enabled: true,
            files: ['right/**/*.*'],
            overrides: [{ filename: '*.jsxx', languageId: 'javascript' }], // codetypo:ignore jsxx
        });
        expect(mergeSettings({}, right)).toEqual(right);
        expect(mergeSettings(left, {})).toEqual(left);
        expect(mergeSettings(left, right)).toEqual(
    ...
    test('tests loading project codetypo.json file', async () => {
        const filename = path.join(pathPackageSamples, 'linked/codetypo-missing.json');
    ...
    test('tests loading a codeTypo.json file', async () => {
        const filename = path.join(pathPackageSamples, 'linked/codetypo-import.json');
        const settings = await readSettings(filename);
        expect(Object.keys(settings)).not.toHaveLength(0);
    ...
    test('tests loading a codeTypo.json with multiple imports file', async () => {
        const filename = path.join(pathPackageSamples, 'linked/codetypo-imports.json');
        const settings = await readSettings(filename);
        expect(Object.keys(settings)).not.toHaveLength(0);
        expect(settings.words).toEqual(expect.arrayContaining(['import']));
    ...
    test('tests loading a codeTypo.json with a missing import file', async () => {
        const filename = path.join(pathPackageSamples, 'linked/codetypo-import-missing.json');
        const settings = await readSettings(filename);
...
function cf(settings: CodeTypoSettingsWST, url: URL | string = import.meta.url): CodeTypoConfigFile {
    return new CodeTypoConfigFileInMemory(toURL(url), settings);
}
