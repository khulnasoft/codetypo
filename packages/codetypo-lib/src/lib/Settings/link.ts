...
import type { CodeTypoSettingsWithSourceTrace } from '@codetypo/codetypo-types';
...
export async function listGlobalImports(): Promise<ListGlobalImportsResults> {
    ...
    const list = (await resolveImports(globalSettings)).map(({ filename, settings, error }) => ({
        ...
        package: findPackageForCodeTypoConfig(Path.dirname(filename)),
    }));
...
function normalizeImports(imports: CodeTypoSettingsWithSourceTrace['import']): string[] {
    return typeof imports === 'string' ? [imports] : imports || [];
}

function resolveImports(s: CodeTypoSettingsWithSourceTrace): Promise<ResolveSettingsResult[]> {
    const imported = normalizeImports(s.import);

    return Promise.all(imported.map(resolveSettings));
}

function findPackageForCodeTypoConfig(pathToConfig: string): NodePackage | undefined {
    try {
