...
import type { CodeTypoUserSettings, GlobDef, LanguageSetting, ReporterSettings } from '@codetypo/codetypo-types';
...
export function normalizeRawConfig(config: CodeTypoUserSettings | NormalizableFields) {
...
type NormalizeDictionaryDefsParams = OptionalOrUndefined<
    Pick<CodeTypoUserSettings, 'dictionaryDefinitions' | 'languageSettings'>
>;
...
type NormalizeOverrides = Pick<CodeTypoUserSettings, 'globRoot' | 'overrides'>;
type NormalizeOverridesResult = Pick<CodeTypoUserSettings, 'overrides'>;
...
type NormalizeReporters = Pick<CodeTypoUserSettings, 'reporters'>;
...
type NormalizeGitignoreRoot = Pick<CodeTypoUserSettings, 'gitignoreRoot'>;
...
interface NormalizeSettingsGlobs {
    files?: CodeTypoUserSettings['files'];
    globRoot?: CodeTypoUserSettings['globRoot'];
    ignorePaths?: CodeTypoUserSettings['ignorePaths'];
}
...
export function normalizeCacheSettings(
    settings: Pick<CodeTypoUserSettings, 'cache'>,
    pathToSettingsFile: URL,
): Pick<CodeTypoUserSettings, 'cache'> {
