...
describe('Validate PnPLoader', () => {
    ...
    test('pnpLoader multiple .pnp.js', async () => {
        ...
        const dictLocationMed = resolveFrom(fsPath(uriYarn2TestMed), '@codetypo/dict-medicalterms/codetypo-ext.json');
        expect(dictLocationMed).toEqual(expect.stringContaining('codetypo-ext.json'));

        // Make sure we can load the science dictionary.
        const dictLocationSci = resolveFrom(
            fsPath(uriYarn2TestSci),
            '@codetypo/dict-scientific-terms-us/codetypo-ext.json',
        );
        expect(dictLocationSci).toEqual(expect.stringContaining('codetypo-ext.json'));
    });
    ...
    test('pnpLoader pass 2', async () => {
        ...
        const dictLocation = resolveFrom(fsPath(uriYarn2TestMed), '@codetypo/dict-medicalterms/codetypo-ext.json');
        expect(dictLocation).toEqual(expect.stringContaining('codetypo-ext.json'));
    });
