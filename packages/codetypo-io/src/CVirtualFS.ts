...
export function createVirtualFS(codetypoIO?: CodeTypoIO): VirtualFS {
    const codetypo = codetypoIO || getDefaultCodeTypoIO();
    const vfs = new CVirtualFS();
    vfs.registerFileSystemProvider(codetypoIOToFsProvider(codetypo));
    return vfs;
}
