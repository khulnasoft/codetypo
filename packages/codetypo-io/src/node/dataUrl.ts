...
export function guessMimeType(filename: string): GuessMimeTypeResult | undefined {
    if (filename.endsWith('.trie')) return { mimeType: 'application/vnd.codetypo.dictionary+trie', encoding: 'utf-8' };
    if (filename.endsWith('.trie.gz')) return { mimeType: 'application/vnd.codetypo.dictionary+trie.gz' };
