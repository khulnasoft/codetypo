...
export function codetypoIOToFsProvider(codetypoIO: CodeTypoIO): VFileSystemProvider {
    ...
    const name = 'CodeTypoIO';
    const supportedProtocols = new Set(['file:', 'http:', 'https:']);
    const fs: VProviderFileSystem = {
        providerInfo: { name },
        stat: (url) => codetypoIO.getStat(url),
        readFile: (url, options) => codetypoIO.readFile(url, options),
        readDirectory: (url) => codetypoIO.readDirectory(url),
        writeFile: (file) => codetypoIO.writeFile(file.url, file.content),
        dispose: () => undefined,
        capabilities,
        getCapabilities(url) {
