...
const pkgCodeTypoLibUrl = new URL('codetypo-lib/', packagesUrl);
...
const pkgCodeTypoLib = fileURLToPath(pkgCodeTypoLibUrl);
...
describe('GitIgnoreServer', () => {
    ...
    test.each`
        ...
        ${p(pkgCodeTypoLib)}       | ${[pkg]}          | ${[gitRoot, pkgCodeTypoLib]}
        ${p(pkgCodeTypoLib)}       | ${[packages]}     | ${[pkgCodeTypoLib]}
