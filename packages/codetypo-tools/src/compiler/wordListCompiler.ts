...
import { opAppend, opMap, pipe } from '@codetypo/codetypo-pipe/sync';
...
export async function compileWordList(
    ...
    const directivesLines = directives.map((a) => `# codetypo-dictionary: ${a}`);
    ...
    return async (words: Iterable<string>) => {
        ...
        await target(
            Trie.serializeTrie(trie, {
                base,
                comment: 'Built by codetypo-tools.',
                version,
            }),
        );
