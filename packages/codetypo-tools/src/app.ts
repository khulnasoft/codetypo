...
function addCompileOptions(compileCommand: Command): Command {
    return compileCommand
        ...
        .option(
            '--init',
            'Create a build command `codetypo-tools.config.yaml` file based upon the options given instead of building.',
        );
}
...
export async function run(program: Command, argv: string[], flags?: FeatureFlags): Promise<void> {
    ...
    addCompileOptions(
        program
            ...
            .description(
                'Compile words lists or Hunspell dictionary into trie files used by codetypo.\nAlias of `compile --trie`',
            ),
