// codetypo:ignore TSESTree
import assert from 'node:assert';
import * as path from 'node:path';

import { toFileDirURL, toFileURL } from '@codetypo/url';
import type { TSESTree } from '@typescript-eslint/types';
import type { CodeTypoSettings, TextDocument, ValidationIssue } from 'codetypo-lib';
import {
    createTextDocument,
    DocumentValidator,
    extractImportErrors,
...
const defaultSettings: CodeTypoSettings = {
...
function calcInitialSettings(options: WorkerOptions): CodeTypoSettings {
    const { customWordListFile, codetypo, cwd } = options;

    const settings: CodeTypoSettings = {
        ...defaultSettings,
        ...codetypo,
        words: codetypo?.words || [],
        ignoreWords: codetypo?.ignoreWords || [],
        flagWords: codetypo?.flagWords || [],
    };

    if (options.configFile) {
        const optionCodetypoImport = options.codetypo?.import;
        const importConfig =
            typeof optionCodetypoImport === 'string'
                ? [optionCodetypoImport]
                : Array.isArray(optionCodetypoImport)
                  ? optionCodetypoImport
                  : [];
...
function resolveDictionaryPaths(defs: CodeTypoSettings['dictionaryDefinitions'], cwd: string) {
    if (!defs) return;
...
async function reportConfigurationErrors(config: CodeTypoSettings, knownConfigErrors: Set<string>): Promise<Error[]> {
    const errors: Error[] = [];
