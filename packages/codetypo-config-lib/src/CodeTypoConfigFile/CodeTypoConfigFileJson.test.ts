...
import { CodeTypoConfigFileJson } from './CodeTypoConfigFileJson.js';

describe('CodeTypoConfigFileJson', () => {
    ...
    test('should serialize the settings to JSON', () => {
        const configFile = new CodeTypoConfigFileJson(url, settings);
    ...
    test('should serialize and preserve indent', () => {
        ...
        const configFile = CodeTypoConfigFileJson.parse({ url, content });
        const serialized = configFile.serialize();
        expect(serialized).toEqual(content);
    });

    test('should parse a JSON file into CodeTypoConfigFileJson object', () => {
        const json = `{
            // This is a comment
            "language": "en",
            "ignoreWords": ["foo", "bar"]
        ...
        const configFile = CodeTypoConfigFileJson.parse(file);
    ...
    test('should throw an error when parsing an invalid JSON file', () => {
        ...
        const file = createTextFile(url, json);
        expect(() => CodeTypoConfigFileJson.parse(file)).toThrowError();
    });
});
