...
import { CodeTypoConfigFileInMemory } from './CodeTypoConfigFile/index.js';
...
export interface CodeTypoConfigFileReaderWriter {
    readonly io: IO;
    readonly middleware: SerializerMiddleware[];
    readonly loaders: FileLoaderMiddleware[];
    readConfig(uri: URL | string): Promise<CodeTypoConfigFile>;
    writeConfig(configFile: CodeTypoConfigFile): Promise<TextFileRef>;
    clearCachedFiles(): void;
    setUntrustedExtensions(ext: readonly string[]): this;
    setTrustedUrls(urls: readonly (URL | string)[]): this;
    toCodeTypoConfigFile(configFile: ICodeTypoConfigFile): CodeTypoConfigFile;
    /**
...
export class CodeTypoConfigFileReaderWriterImpl implements CodeTypoConfigFileReaderWriter {
    /**
    ...
    toCodeTypoConfigFile(configFile: ICodeTypoConfigFile): CodeTypoConfigFile {
        return configFile instanceof CodeTypoConfigFile
            ? configFile
            : new CodeTypoConfigFileInMemory(configFile.url, configFile.settings);
    }
    ...
    serialize(configFile: ICodeTypoConfigFile): string {
        const serializer = getSerializer(this.middleware);
        return serializer(configFile);
    }

    async writeConfig(configFile: ICodeTypoConfigFile): Promise<TextFileRef> {
