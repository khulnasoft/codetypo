...
import { toFileDirURL, toFilePathOrHref, toFileURL } from '@codetypo/url';
import type { BufferEncoding } from 'codetypo-io';
import { readFileText as cioReadFile, toURL } from 'codetypo-io';
import type { CodeTypoUserSettings, Document, Issue } from 'codetypo-lib';
import * as codetypo from 'codetypo-lib';
import { fileToDocument, isBinaryFile as isUriBinaryFile } from 'codetypo-lib';
import getStdin from 'get-stdin';

import { CodeTypoConfigFile } from '../options.js';
...
export async function readConfig(
    configFile: string | CodeTypoConfigFile | undefined,
    root: string | undefined,
): Promise<ConfigInfo> {
    if (configFile) {
        const cfgFile = typeof configFile === 'string' ? await readConfigHandleError(configFile) : configFile;
        const config = await codetypo.resolveConfigFileImports(cfgFile);
        const source = toFilePathOrHref(cfgFile.url);
        return { source, config };
    }
    const config = await codetypo.searchForConfig(root);
    return { source: config?.__importRef?.filename || 'None found', config: config || {} };
}

export function readConfigFile(filename: string | URL): Promise<CodeTypoConfigFile> {
    return codetypo.readConfigFile(filename);
}

async function readConfigHandleError(filename: string | URL): Promise<CodeTypoConfigFile> {
    try {
        return await readConfigFile(filename);
    } catch (e) {
        const settings: codetypo.CodeTypoSettingsWithSourceTrace = {
            __importRef: {
...
export type Perf = codetypo.SpellCheckFilePerf;
