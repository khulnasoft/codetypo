...
import { toFileDirURL, toFilePathOrHref, toFileURL, urlRelative } from '@codetypo/url';
import type { ChalkInstance } from 'chalk';
import { Chalk } from 'chalk';
import { makeTemplate } from 'chalk-template';
import type { ImportError, SpellCheckFilePerf, SpellingDictionaryLoadError } from 'codetypo-lib';
import { isSpellingDictionaryLoadError } from 'codetypo-lib';
...
function reportProgress(io: IO, p: ProgressItem, cwdURL: URL, options: CodeTypoReporterConfiguration) {
...
function reportProgressFileComplete(io: IO, p: ProgressFileComplete, cwd: URL, options: CodeTypoReporterConfiguration) {
...
export function getReporter(options: ReporterOptions, config?: CodeTypoReporterConfiguration): FinalizedReporter {
    const perfStats = {
        filesProcessed: 0,
        filesSkipped: 0,
        filesCached: 0,
    ...
    /*
     * Turn off repeated issues see https://github.com/khulnasoft/codetypo/pull/6058
    ...
    const resultEmitter = (result: RunResult) => {
        ...
        const summaryMessage = `CodeTypo\u003A Files checked: ${files}${cachedFilesText}, Issues found: ${issues} in ${numFilesWidthIssuesText}${withErrorsText}.`;
