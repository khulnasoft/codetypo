...
import type { Issue, RunResult } from '@codetypo/codetypo-types';
...
describe('Validate the Application', () => {
    ...
    test(
        ...
        async () => {
            const result = await trace(['apple'], { config: 'samples/codetypo-missing-dict.json' });
            expect(result.length).toBeGreaterThan(2);
    ...
    test(
        ...
        async () => {
            ...
            // codetypo:ignore texxt
            const text = `
                This is some texxt to test out reding from stdin.
                codetypo:ignore badspellingintext
...
describe('Linter File Caching', () => {
    ...
    `('lint caching with $root $comment', async ({ runs, root }: TestCase) => {
        const reporter = new InMemoryReporter();
        const cacheLocation = tempLocation('.codetypocache');
...
function sampleTests(): SampleTest[] {
    ...
    return [
        ...
        {
            file: path.join(pathPackageRoot, '../codetypo-lib/samples/src/drives.ps1'),
            issues: ['Woude', 'Woude'],
        },
