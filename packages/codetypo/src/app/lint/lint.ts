...
import {
    ...
    getDictionary,
    isBinaryFile as codetypoIsBinaryFile,
    setLogger,
    shouldCheckDocument,
    spellCheckDocument,
    Text as codetypoText,
} from 'codetypo-lib';
...
import type { CreateCacheSettings, CodeTypoLintResultCache } from '../util/cache/index.js';
...
export async function runLint(cfg: LintRequest): Promise<RunResult> {
    ...
    const logDictRequests = truthy(getEnvironmentVariable('CODETYPO_ENABLE_DICTIONARY_LOGGING'));
    if (logDictRequests) {
        codetypoDictionaryDebug.cacheDictionaryEnableLogging(true);
    }
    ...
    function prefetch(filename: string, configInfo: ConfigInfo, cache: CodeTypoLintResultCache): PrefetchFileResult {
    ...
    async function processFile(
        ...
        try {
            ...
            result.issues = codetypoText.calculateTextDocumentOffsets(doc.uri, text, r.issues).map(mapIssue);
        } catch (e) {
    ...
    function calcDependencies(config: CodeTypoSettings): ConfigDependencies {
        const { configFiles, dictionaryFiles } = extractDependencies(config);

        return { files: [...configFiles, ...dictionaryFiles] };
    }

    async function reportConfigurationErrors(config: CodeTypoSettings): Promise<number> {
    ...
    async function run(): Promise<RunResult> {
        if (cfg.options.root) {
            setEnvironmentVariable(ENV_CODETYPO_GLOB_ROOT, cfg.root);
        }
        ...
        const reporterConfig: CodeTypoReporterConfiguration = util.clean({
...
async function determineFilesToCheck(
    ...
    function isExcluded(filename: string, globMatcherExclude: GlobMatcher) {
        if (codetypoIsBinaryFile(toFileURL(filename))) {
            return true;
        }
        const { root } = cfg;
...
async function writeDictionaryLog() {
    const fieldsCsv = getEnvironmentVariable('CODETYPO_ENABLE_DICTIONARY_LOG_FIELDS') || 'time, word, value';
    const fields = fieldsCsv.split(',').map((f) => f.trim());
    const header = fields.join(', ') + '\n';
    const lines = codetypoDictionaryDebug
    ...
    const filename = getEnvironmentVariable('CODETYPO_ENABLE_DICTIONARY_LOG_FILE') || 'codetypo-dictionary-log.csv';
