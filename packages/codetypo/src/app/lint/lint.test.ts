...
describe('Linter Validation Tests', () => {
    ...
    const optionsRootCodeTypoJson = { root, config: j(root, 'codetypo.json') };

    // codetypo:ignore Tufte checkedd
    test.each`
        ...
        ${['*.txt']}        | ${{ root: failFastSamples, config: j(samples, 'fail-fast/fail-fast-codetypo.json') }}                    | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: oc({ length: 1 }) })}
        ${['*.txt']}        | ${{ root: failFastSamples, failFast: true }}                                                           | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: oc({ length: 1 }) })}
        ${['*.txt']}        | ${{ root: failFastSamples, failFast: false, config: j(samples, 'fail-fast/fail-fast-codetypo.json') }}   | ${oc({ errors: 0, files: 2 })} | ${oc({ errorCount: 0, errors: [], issues: oc({ length: 2 }) })}
        ...
        ${['**']}           | ${{ root: samples, config: j(samples, 'codetypo-not-found.json') }}                                      | ${oc({ errors: 1, files: 0 })} | ${oc({ errorCount: 1, errors: [expect.any(Error)], issues: [] })}
        ${['**']}           | ${{ root: samples, config: j(samples, 'linked/codetypo-import-missing.json') }}                          | ${oc({ errors: 1, files: 0 })} | ${oc({ errorCount: 1, errors: [expect.any(Error)], issues: [] })}
        ${['**/ebook.tex']} | ${{ root: samples, config: j(samples, 'codetypo-missing-dict.json') }}                                   | ${oc({ errors: 0, files: 0 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${['**/ebook.tex']} | ${{ root: samples, config: j(samples, 'linked/codetypo-import.json') }}                                  | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${[]}               | ${{ root, config: j(root, 'codetypo.json'), fileLists: [filesToCheck], dot: true }}                      | ${oc({ errors: 0, files: 2 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${['**/*.md']}      | ${{ root, config: j(root, 'codetypo.json'), fileLists: [filesToCheck] }}                                 | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${['**/*.ts']}      | ${{ root, config: j(root, 'codetypo.json'), fileLists: [filesToCheck] }}                                 | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${[]}               | ${{ root, config: j(root, 'codetypo.json'), fileLists: ['missing-file.txt'] }}                           | ${oc({ errors: 1, files: 0 })} | ${oc({ errorCount: 1, errors: [expect.any(Error)], issues: [] })}
        ${[]}               | ${{ root, config: j(root, 'codetypo.json'), fileLists: [filesToCheckWithMissing] }}                      | ${oc({ errors: 0, files: 3 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${[]}               | ${{ root, config: j(root, 'codetypo.json'), fileLists: [filesToCheckWithMissing], mustFindFiles: true }} | ${oc({ errors: 1, files: 3 })} | ${oc({ errorCount: 1, errors: [expect.anything()], issues: [] })}
        ${["'**'"]}         | ${{ root, config: j(root, 'codetypo.json'), mustFindFiles: true }}                                       | ${oc({ errors: 0, files: 0 })} | ${oc({ errorCount: 1, errors: [expect.any(CheckFailed)], issues: [] })}
        ${['**']}           | ${{ root: j(configSamples, 'yaml-regexp') }}                                                           | ${oc({ errors: 0, files: 2 })} | ${oc({ errorCount: 0, errors: [], issues: [oc({ text: 'checkedd' })] })}
        ${[]}               | ${{ ...optionsRootCodeTypoJson, files: ['README.md', 'LICENSE'], dot: true }}                            | ${oc({ errors: 0, files: 2 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${['*.md']}         | ${{ ...optionsRootCodeTypoJson, files: ['README.md', 'LICENSE'], dot: true }}                            | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${[]}               | ${{ ...optionsRootCodeTypoJson, files: ['README.md', 'missing.txt'], dot: true, mustFindFiles: false }}  | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${[]}               | ${{ ...optionsRootCodeTypoJson, files: ['README.md', 'missing.txt'], dot: true, mustFindFiles: true }}   | ${oc({ errors: 1, files: 2 })} | ${oc({ errorCount: 1, errors: [expect.anything()], issues: [] })}
        ${[]}               | ${{ ...optionsRootCodeTypoJson, files: ['../../README.md'], dot: true }}                                 | ${oc({ errors: 0, files: 1 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${[]}               | ${{ ...optionsRootCodeTypoJson, files: ['../../resources/patreon.png' /* skip binary */], dot: true }}   | ${oc({ errors: 0, files: 0 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
        ${['**/*.md']}      | ${{ root: './fixtures/issue-6025', config: './fixtures/issue-6025/nested/codetypo.config.yaml' }}        | ${oc({ errors: 0, files: 2 })} | ${oc({ errorCount: 0, errors: [], issues: [] })}
    ...
    `('runLint $files $options', async ({ files, options }) => {
        const reporter = new InMemoryReporter();
        process.env[environmentKeys.CODETYPO_ENABLE_DICTIONARY_LOGGING] = 'true';
        process.env[environmentKeys.CODETYPO_ENABLE_DICTIONARY_LOG_FILE] = 'stdout';
        process.env[environmentKeys.CODETYPO_ENABLE_DICTIONARY_LOG_FIELDS] = 'word, value';
