import { opMap, opTap, pipeAsync, toAsyncIterable } from '@codetypo/codetypo-pipe';
import type { CodeTypoReporter, RunResult } from '@codetypo/codetypo-types';
import type { CheckTextInfo, FeatureFlags, SuggestionsForWordResult, TraceWordResult } from 'codetypo-lib';
import {
    checkTextDocument,
    getDefaultSettings,
...
export type { TraceResult } from 'codetypo-lib';
export { IncludeExcludeFlag } from 'codetypo-lib';

export type AppError = NodeJS.ErrnoException;

export function lint(fileGlobs: string[], options: LinterCliOptions, reporter?: CodeTypoReporter): Promise<RunResult> {
    options = fixLegacy(options);
    const reporterOptions: CodeTypoReporterConfiguration = { ...options, console };
