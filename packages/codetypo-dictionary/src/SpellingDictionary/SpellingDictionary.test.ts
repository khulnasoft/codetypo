import { buildITrieFromWords } from 'codetypo-trie-lib';
...
describe('Verify building Dictionary', () => {
    ...
    test('compounds from word list', async () => {
        ...
        expect(dict.has('APPles')).toBe(true); // codetypo:disable-line
        // codetypo:ignore applebanana applebananas applebananaorange
    ...
    test('case-sensitive word list', async () => {
        ...
        expect(dict.has('mcgreyer', useCase)).toBe(false); // codetypo:disable-line
        // We do not support mixed case as all caps matching at this point.
        expect(dict.has('MCGREYER', useCase)).toBe(false); // codetypo:disable-line
        expect(dict.has('MCGREYER', ignoreCase)).toBe(true); // codetypo:disable-line
    });
...
// codetypo:words métro Rhône Köln Düsseldorf
