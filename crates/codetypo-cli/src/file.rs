...
#[derive(Debug, Clone, Copy)]
pub struct FixCodetypo;

impl FileChecker for FixCodetypo {
    fn check_file(
        &self,
        path: &std::path::Path,
        explicit: bool,
...
#[derive(Debug, Clone, Copy)]
pub struct DiffCodetypo;

impl FileChecker for DiffCodetypo {
    fn check_file(
        &self,
        path: &std::path::Path,
        explicit: bool,
        ...
        // Match FixCodetypo ordering for easy diffing.
        ...
        if policy.check_files {
            ...
            if !explicit && !policy.binary && content_type.is_binary() {
                ...
                for word in policy.tokenizer.parse_bytes(&buffer) {
                    ...
                    // of Codetypo for comparison purposes.  We don't really get much out of it
                    // anyway.
                    let line_num = 0;
                    let msg = report::Parse {
                        context: Some(report::FileContext { path, line_num }.into()),
...
) -> impl Iterator<Item = codetypo::Typo<'a>> {
    let mut ignores: Option<Ignores> = None;

    codetypo::check_str(buffer, policy.tokenizer, policy.dict).filter(move |typo| {
        !ignores
            .get_or_insert_with(|| Ignores::new(buffer.as_bytes(), policy.ignore))
...
fn check_bytes<'a>(
    ...
    codetypo::check_bytes(buffer, policy.tokenizer, policy.dict).filter(move |typo| {
        !ignores
            .get_or_insert_with(|| Ignores::new(buffer, policy.ignore))
            .is_ignored(typo.span())
    })
...
fn extract_fix<'t>(typo: &'t codetypo::Typo<'t>) -> Option<&'t str> {
    match &typo.corrections {
        codetypo::Status::Corrections(c) if c.len() == 1 => Some(c[0].as_ref()),
        _ => None,
    }
}

fn is_fixable(typo: &codetypo::Typo<'_>) -> bool {
...
fn walk_entry(
    ...
    {
        log::debug!(
            "{}: skipping potential config file as it may have codetypo",
            entry.path().display()
        );
        return Ok(());
    }
...
mod test {
    ...
    fn fix_simple(line: &str, corrections: Vec<(usize, &'static str, &'static str)>) -> String {
        ...
        let corrections = corrections
            .into_iter()
            .map(|(byte_offset, typo, correction)| codetypo::Typo {
                byte_offset,
                typo: typo.into(),
                corrections: codetypo::Status::Corrections(vec![correction.into()]),
            });
