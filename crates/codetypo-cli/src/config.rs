...
impl Config {
    ...
    pub fn from_file(path: &std::path::Path) -> Result<Option<Self>, anyhow::Error> {
        ...
        if path.file_name().unwrap() == CARGO_TOML {
            ...
            let codetypo = config
                .workspace
                .and_then(|w| w.metadata.codetypo)
                .or(config.package.and_then(|p| p.metadata.codetypo));

            if let Some(codetypo) = codetypo {
                Ok(Some(codetypo))
            } else {
                log::debug!(
                    "No `package.metadata.codetypo` section found in `{CARGO_TOML}`, skipping"
                );
            ...
            if let Some(codetypo) = config.tool.codetypo {
                Ok(Some(codetypo))
            } else {
                log::debug!("No `tool.codetypo` section found in `{PYPROJECT_TOML}`, skipping");
