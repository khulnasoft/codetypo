...
pub(crate) struct MessageStatus<'r> {
    codetypo_found: atomic::AtomicBool,
...
impl<'r> MessageStatus<'r> {
    pub(crate) fn new(reporter: &'r dyn Report) -> Self {
        Self {
            codetypo_found: atomic::AtomicBool::new(false),
            errors_found: atomic::AtomicBool::new(false),
            reporter,
        }
    }

    pub(crate) fn codetypo_found(&self) -> bool {
        self.codetypo_found.load(atomic::Ordering::Relaxed)
    }
...
impl Report for MessageStatus<'_> {
    fn report(&self, msg: Message<'_>) -> Result<(), std::io::Error> {
        if msg.is_typo() {
            self.codetypo_found.store(true, atomic::Ordering::Relaxed);
        }
        if msg.is_error() {
...
fn print_brief_correction(msg: &Typo<'_>) -> Result<(), std::io::Error> {
    ...
    match &msg.corrections {
        codetypo::Status::Valid => {}
        codetypo::Status::Invalid => {
            let divider = ":";
            writeln!(
                stdout().lock(),
...
fn type_to_sarif_message(msg: &Typo<'_>) -> Option<String> {
    match &msg.corrections {
        codetypo::Status::Valid => None,
        codetypo::Status::Invalid => Some(format!("`{}` is disallowed", msg.typo)),
        codetypo::Status::Corrections(corrections) => Some(format!(
            "`{}` should be {}",
            msg.typo,
