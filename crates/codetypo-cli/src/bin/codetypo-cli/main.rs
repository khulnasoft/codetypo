...
use codetypo_cli::report::Report;
...
fn run_dump_config(args: &args::Args, output_path: &std::path::Path) -> proc_exit::ExitResult {
    ...
    let storage = codetypo_cli::policy::ConfigStorage::new();
    let mut engine = codetypo_cli::policy::ConfigEngine::new(&storage);
    engine.set_isolated(args.isolated);

    let mut overrides = codetypo_cli::config::Config::default();
    if let Some(path) = args.custom_config.as_ref() {
        let custom = codetypo_cli::config::Config::from_file(path)
    ...
    let mut defaulted_config = codetypo_cli::config::Config::from_defaults();
...
fn run_checks(args: &args::Args) -> proc_exit::ExitResult {
    ...
    'path: for path in file_list.as_ref().unwrap_or(&args.path) {
        ...
        let selected_checks: &dyn codetypo_cli::file::FileChecker = if args.files {
            ...
            &codetypo_cli::file::FixCodetypo
        } else if args.diff {
            &codetypo_cli::file::DiffCodetypo
        } else {
            &codetypo_cli::file::Codetypo
        };

        if single_threaded {
            codetypo_cli::file::walk_path(
                walk.build(),
                selected_checks,
                &engine,
                reporter,
            ...
            codetypo_cli::file::walk_path_parallel(
                walk.build_parallel(),
                selected_checks,
                &engine,
                reporter,
        ...
        if status_reporter.codetypo_found() {
            codetypo_found = true;
        }
    ...
    } else if codetypo_found {
        // Can;'t use `Failure` since its so prevalent, it could be easy to get a
        // `Failure` from something else and get it mixed up with codetypo.
        //
