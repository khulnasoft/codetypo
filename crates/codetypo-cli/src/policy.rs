...
pub struct ConfigEngine<'s> {
    ...
    tokenizer: Intern<codetypo::tokens::Tokenizer>,
...
impl<'s> ConfigEngine<'s> {
    ...
    fn get_tokenizer(&self, file: &FileConfig) -> &codetypo::tokens::Tokenizer {
        self.tokenizer.get(file.tokenizer)
    }

    fn get_dict(&self, file: &FileConfig) -> &dyn codetypo::Dictionary {
    ...
    fn init_file_config(&mut self, engine: crate::config::EngineConfig) -> FileConfig {
        ...
        let tokenizer = codetypo::tokens::TokenizerBuilder::new()
...
pub struct Policy<'t, 'd, 'i> {
    ...
    pub binary: bool,
    pub tokenizer: &'t codetypo::tokens::Tokenizer,
    pub dict: &'d dyn codetypo::Dictionary,
...
static DEFAULT_TOKENIZER: codetypo::tokens::Tokenizer = codetypo::tokens::Tokenizer::new();
