...
impl BuiltIn {
    ...
    pub fn correct_ident<'s>(
        &'s self,
        ident_token: codetypo::tokens::Identifier<'_>,
    ) -> Option<Status<'s>> {
...
impl BuiltIn {
    ...
    fn correct_word_with_dict(&self, word: UniCase<&str>) -> Option<&'static [&'static str]> {
        codetypo_dict::WORD.find(&word).copied()
    }
}
...
impl BuiltIn {
    ...
    fn is_vars_enabled(&self) -> bool {
        ...
        debug_assert!(codetypo_vars::NO_INVALID);
        self.locale.is_some()
    }

    fn select_variant(
        &self,
        vars: &'static [(u8, &'static codetypo_vars::VariantsMap)],
    ) -> Status<'static> {
        let var = vars[0];
        let var_categories = unsafe {
            // Code-genned from a checked category-set, so known to be safe
            codetypo_vars::CategorySet::from_bits_unchecked(var.0)
        };
        if let Some(locale) = self.locale {
            if var_categories.contains(locale) {
                ...
                Status::Corrections(
                    codetypo_vars::corrections(locale, *var.1)
                        .iter()
                        .copied()
                        .map(Cow::Borrowed)
                        .collect(),
...
impl codetypo::Dictionary for BuiltIn {
    fn correct_ident<'s>(&'s self, ident: codetypo::tokens::Identifier<'_>) -> Option<Status<'s>> {
        BuiltIn::correct_ident(self, ident)
    }

    fn correct_word<'s>(&'s self, word: codetypo::tokens::Word<'_>) -> Option<Status<'s>> {
...
impl<'i, 'w, D: codetypo::Dictionary> Override<'i, 'w, D> {
...
impl<D: codetypo::Dictionary> codetypo::Dictionary for Override<'_, '_, D> {
    fn correct_ident<'s>(&'s self, ident: codetypo::tokens::Identifier<'_>) -> Option<Status<'s>> {
    ...
    fn correct_word<'s>(&'s self, word_token: codetypo::tokens::Word<'_>) -> Option<Status<'s>> {
        if word_token.case() == Case::None {
            return None;
        }
...
mod test {
    ...
    fn test_dict_correct() {
        ...
        let correction = dict.correct_word(codetypo::tokens::Word::new_unchecked(
            "finallizes",
            Case::Lower,
            0,
        ));
