---
title: 'Getting Started with CodeTypo'
categories: docs
# parent: Docs
nav_order: 2
---
...
# Adding CodeTypo to an existing project

In the steps below we will create a codetypo configuration file and setup a single custom dictionary for the project.
...
CodeTypo can use JSON, Yaml, and JavaScript files for configuration. It automatically searches for one of the following: `codetypo.json`, `codetypo.config.yaml`, `codetypo.config.cjs`.

For now choose to use either JSON or Yaml. Below are examples of each that include a custom dictionary definition. Both of them are equivalent. If you have both, CodeTypo will look for the `.json` file first.
...
1. Have CodeTypo populate it with the words from your project.

   ```sh
   echo "# New Words" >> project-words.txt
   codetypo --words-only --unique "**/*.md" | sort --ignore-case >> project-words.txt
   ```
...
1. Fix spelling issues.
   ...
   ```sh
   codetypo --no-progress --show-suggestions --show-context "**/*.md"
   ```
...
- [Understanding CodeTypo Globs](./globs.md)
