...
- Search parents of specified file / directory for one of `codetypo.toml`, `_codetypo.toml`, `.codetypo.toml`, `Cargo.toml`, or `pyproject.toml`.
  - In `pyproject.toml`, the below fields must be under the `[tool.codetypo]` section. If this section does not
    exist, the config file will be skipped.
  - In `Cargo.toml`, the below fields must be under either `[workspace.metadata.codetypo]` or `[package.metadata.codetypo]`
...
| files.extend-exclude   | --exclude         | list of strings | \- | Codetypo-specific ignore globs (gitignore syntax). |
